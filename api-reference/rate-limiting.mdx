---
title: 'Rate Limiting'
description: 'Entenda como funciona o rate limiting na API Sync Pay'
icon: 'gauge'
---

## Visão Geral

A API Sync Pay implementa rate limiting para garantir estabilidade, segurança e uso justo dos recursos. Diferentes tipos de rotas têm limites diferentes baseados no método de autenticação.
...
for i in {1..25}; do
  curl -H "X-API-Key: sua_key" https://api.syncpay.com.br/api/v1/user/balance
done
...
<CardGroup cols={2}>
  ...
  <Card title="Saques" icon="money-bill-transfer" href="/api-reference/endpoint/withdraws/create">
